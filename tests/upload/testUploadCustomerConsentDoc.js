...
const fileMetadata = {
    type: 'CUSTOMER_CONSENT_DOC',
    filename: path.basename(uploadfilePath)
}

async function test() {
    let client = await DvaraGold.Client(config)
    let uploadResponse = await client.getCustomerDocumentUploadURLForConsent(extCustomerId, fileMetadata);
    ...
    let getFileFromFetchPath = await client.getCustomerFileBasedOnFilePath(uploadResponse.fetchurl);
    console.log(getFileFromFetchPath);
